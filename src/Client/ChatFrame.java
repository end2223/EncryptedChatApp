...
public class ChatFrame extends JFrame {
    ...
    //label nguoi nhan ko phai combobox
    ...
    //xac dinh nguoi ibox va cua so chat 
    ...
    private long Stime;
    private long Etime;
    private int expiryKey = 1; // thoi gian khoa het han(phut)
     ...
     * Create the frame.
     */
    public ChatFrame(User u, DataInputStream dis, DataOutputStream dos) {
        setTitle("MANGO CHAT");
        this.user = u;
        this.username = u.getUsername();
        this.dis = dis;
        this.dos = dos;
        receiver = new Thread(new Receiver(dis,dos));
        receiver.start();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (rVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        // ma hoa
                        String str = new String(selectedFile);
                        str = aes.encrypt(str, user.getGenKey().get(lbReceiver.getText())+"");
                        selectedFile = str.getBytes();
//                        System.out.println(new String(selectedFile));
                        
                        dos.writeUTF("File");
                        //ten nguoi nhan
                        dos.writeUTF(lbReceiver.getText());
                        //duong dan file
                        dos.writeUTF(aes.encrypt(fileChooser.getSelectedFile().getName(), user.getGenKey().get(lbReceiver.getText())+""));
            ...
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    ...
                    if (chatWindow != chatWindows.get(lbReceiver.getText())) {
                        ...
                        chatPanel.validate();
                        ////////////////////////////////////////
                        if(user.getGenKey().containsKey(lbReceiver.getText())==false){
                            try {
                                ...
                                user.setP(df.SinhSNT());
                                user.setG(df.SinhSNT());
                                BigInteger mixKey = df.mixKey(user.getSecret(), user.getG(), user.getP());
                                user.getGenKey().put(lbReceiver.getText(), new BigInteger("-1"));
 
                                dos.writeUTF(user.getP()+"");
                                dos.writeUTF(user.getG()+"");
                                dos.writeUTF(mixKey+"");
        ...
        btnSend.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                try {                  
                    dos.writeUTF("Text");
                    dos.writeUTF(lbReceiver.getText());
                    dos.writeUTF(aes.encrypt(txtMessage.getText(), 
                            user.getGenKey().get(lbReceiver.getText())+""));
                    
                    String a = aes.encrypt(txtMessage.getText(), 
                            user.getGenKey().get(lbReceiver.getText())+"");
                    System.out.println("Ban ma: "+a);
...
//                        System.out.println(user.getGenKey().get(sender));
//                        System.out.println(message);
                        String mes = aes.decrypt(message, user.getGenKey().get(sender)+"");
                        
                        // In tin nhắn lên màn hình chat với người gửi
                        newMessage(sender, mes, false);
                        ...
                        filename = aes.decrypt(filename, user.getGenKey().get(sender)+"");
                        ...
                        byte[] file1 = new byte[file.size()];
                        file1 = file.toByteArray();
//                        System.out.println(new String(file1));
                        String str = new String(file1);
                        str = aes.decrypt(str, user.getGenKey().get(sender)+"");
                        file1 = str.getBytes();
                        // In ra màn hình file đó
                        newFile(sender, filename, file1, false);
                        ...
                        user.getGenKey().remove(sender);
                        user.setP(new BigInteger(p));
                        user.setG(new BigInteger(g));
                        DiffieHellman df = new DiffieHellman();
                        user.getGenKey().put(sender, df.genKey(new BigInteger(mixKey)
                                , user.getSecret(), user.getP()));
                        System.out.println("khoa chung "+user.getGenKey().get(sender));
                        dos.writeUTF("key2");
                        dos.writeUTF(sender);
                        dos.writeUTF(df.mixKey(user.getSecret(), user.getG(), user.getP())+"");
                        dos.flush();
                        
                    }
                    else if(method.equals("key2")){
                        sender = dis.readUTF();
                        String mixKey = dis.readUTF();
                        DiffieHellman df = new DiffieHellman();
                        user.getGenKey().put(sender, df.genKey(new BigInteger(mixKey)
                                , user.getSecret(), user.getP()));
                        System.out.println(df.genKey(new BigInteger(mixKey)
                                , user.getSecret(), user.getP()));
                        
                    }
                    Etime = System.currentTimeMillis() - Stime;
                    if(Etime>=60000*expiryKey){
                        if(user.getGenKey().containsKey(sender)==true){
                            user.getGenKey().remove(sender);
                            try {
                                Stime = System.currentTimeMillis();
                                dos.writeUTF("key");
                                ...
                                user.setG(df.SinhSNT());
                                BigInteger mixKey = df.mixKey(user.getSecret(), user.getG(), user.getP());
                                user.getGenKey().put(sender, new BigInteger("-1"));
